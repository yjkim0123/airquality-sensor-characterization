...
MST requires explicit consideration of measurement uncertainty. We estimate the single-station PM\textsubscript{2.5} measurement uncertainty from co-located station pairs, following the approach of dividing pair-wise RMSE by $\sqrt{2}$ under the assumption that both stations have comparable measurement precision. From 279~pairs within 5~km, the median RMSE is \SI{6.94}{\micro\gram/\meter\cubed}, yielding a single-station uncertainty of $\pm$\SI{4.9}{\micro\gram/\meter\cubed} (median). This uncertainty is concentration-dependent: at low concentrations (${\sim}4$~\si{\micro\gram/\meter\cubed}), the coefficient of variation (CV) reaches 0.379 (uncertainty ${\sim}\pm$\SI{1.6}{\micro\gram/\meter\cubed}), decreasing to CV~$=$~0.035 at moderate levels (${\sim}15$~\si{\micro\gram/\meter\cubed}; uncertainty ${\sim}\pm$\SI{0.5}{\micro\gram/\meter\cubed}), and increasing again at high concentrations (${\sim}57$~\si{\micro\gram/\meter\cubed}; CV~$=$~0.258, uncertainty ${\sim}\pm$\SI{14.8}{\micro\gram/\meter\cubed}). The inter-quartile range of pair-wise RMSE [\num{5.42}, \num{8.79}]~\si{\micro\gram/\meter\cubed} indicates substantial variability in precision across the network. These estimates include both genuine micro-scale spatial variability and instrument measurement error, and thus represent an upper bound on instrument uncertainty alone.
